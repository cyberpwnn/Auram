...
*/
    ...
    float clamped = distance(repcol, taacol)/getLuma(repcol);


    //flicker reduction
    float ldiff     = distance(repcol, scenecol)/getLuma(repcol);
...
float temporal_exp() {
    ...
    float exp_targ  = rcp(get_imageLuma(colortex0));
        exp_targ    = clamp(exp_targ, 4.0, 50.0 + nightVision*15.0);
        exp_targ    = log2(exp_targ / 8.5);    //adjust this
        exp_targ    = 1.2 * pow(2.0, exp_targ);

    return mix(exp_curr, exp_targ, 0.035*(frameTime/0.033));
}
