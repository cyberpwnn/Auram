...
float genLens(vec2 lightPos, float size, float dist,float rough){
	return pow(clamp(max(1.0-length((coord.xy+(lightPos.xy*dist-0.5))*vec2(aspectRatio,1.0)/(size*fovmult)),0.0),0.0,1.0/rough)*rough,4.0);
}
...
float circleDist (vec2 lightPos, float dist, float size) {
	...
	return pow(min(distratio(pos.xy, coord.xy, aspectRatio),size)/size,10.);
}
...
float genAnaLens(vec2 lightPos){
	return pow(max(1.0-length(pow(abs(coord.xy-lightPos.xy-0.5),vec2(0.5,0.8))*vec2(aspectRatio*0.175,2.0))*4.0/fovmult,0.0),2.2);
}
...
vec3 genRainbowHalo(vec2 lightPos) {
    ...
    return color_saturation(vec3((b), pow2(g), pow2(r)), 0.75)*vec3(1.0, 0.8, 0.3)*0.02*mask;
}
...
vec3 genRainbowHalo2(vec2 lightPos) {
    ...
    float r     = sstep(dist, 0.0, 0.33)*(1.0-sstep(dist, 0.33, 0.66));
    float g     = pow2(sstep(dist, 0.0, 0.5)*(1.0-sstep(dist, 0.5, 1.0)));
    float b     = sstep(dist, 0.5, 0.66)*(1.0-sstep(dist, 0.66, 1.0));

    return color_saturation(vec3(r, g, b), 0.6)*vec3(1.0, 0.8, 0.2)*0.04*mask;
}
