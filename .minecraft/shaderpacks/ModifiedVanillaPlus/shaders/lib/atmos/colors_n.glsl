...
void make_colors() {
    light_color[0]  = pow(fogColor, vec3(2.4))*nskylight_luma;

    light_color[1]  = blocklight_col*blocklight_luma*2.0;


    #ifdef skypass
        sky_color    = pow(fogColor, vec3(2.4))*0.3;
    #endif
}