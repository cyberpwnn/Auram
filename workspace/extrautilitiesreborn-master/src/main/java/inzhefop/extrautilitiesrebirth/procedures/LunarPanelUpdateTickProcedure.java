...
public class LunarPanelUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		double energy = 0;
		if (world.canSeeSkyFromBelowWater(BlockPos.containing(x, y, z)) && !(world instanceof Level _lvl && _lvl.isDay())) {
			{
		...
		if (new Object() {
			...
			}.extractEnergySimulate(world, BlockPos.containing(x, y, z), 100000);
			energy = new Object() {
		...
		if (ExtrautilitiesrebirthModBlocks.LUNAR_PANEL.get() == (world.getBlockState(BlockPos.containing(x + 1, y, z))).getBlock()) {
			if (new Object() {
				public boolean canReceiveEnergy(LevelAccessor level, BlockPos pos) {
					AtomicBoolean _retval = new AtomicBoolean(false);
					BlockEntity _ent = level.getBlockEntity(pos);
		...
		if (ExtrautilitiesrebirthModBlocks.LUNAR_PANEL.get() == (world.getBlockState(BlockPos.containing(x, y, z + 1))).getBlock()) {
			if (new Object() {
				public boolean canReceiveEnergy(LevelAccessor level, BlockPos pos) {
					AtomicBoolean _retval = new AtomicBoolean(false);
					BlockEntity _ent = level.getBlockEntity(pos);
